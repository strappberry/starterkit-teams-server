...
<div @class([
    ...
    <select wire:model.live="perPage" id="{{ $tableName }}-perPage"
        {{ $attributes->merge($component->getPerPageFieldAttributes())->class([
                ...
                'block w-full rounded-md shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 focus:ring focus:ring-opacity-50' =>
                    $this->isTailwind && $component->getPerPageFieldAttributes()['default-styling'],
                'border-gray-300 focus:border-primary-300 focus:ring-primary-200 dark:bg-gray-700 dark:text-white dark:border-gray-600' =>
                    $this->isTailwind && $component->getPerPageFieldAttributes()['default-colors'],
