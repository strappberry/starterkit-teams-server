...
@if ($this->bulkActionsAreEnabled() && $this->hasBulkActions())
    ...
    @if ($isTailwind)
        <x-livewire-tables::table.tr.plain x-cloak x-show="selectedItems.length > 0 && !currentlyReorderingStatus"
            wire:key="{{ $tableName }}-bulk-select-message" class="bg-primary-50 dark:bg-gray-900 dark:text-white">
            <x-livewire-tables::table.td.plain :colspan="$colspan">
                <template x-if="selectedItems.length == paginationTotalItemCount || selectAllStatus">
                    <div wire:key="{{ $tableName }}-all-selected">
                        ...
                        <button x-on:click="clearSelected" wire:loading.attr="disabled" type="button"
                            class="ml-1 text-sm font-medium leading-5 text-blue-600 text-gray-700 underline transition duration-150 ease-in-out focus:text-gray-800 focus:underline focus:outline-none dark:text-white dark:hover:text-gray-400">
                ...
                <template x-if="selectedItems.length !== paginationTotalItemCount && !selectAllStatus">
                    <div wire:key="{{ $tableName }}-some-selected">
                        ...
                        <button x-on:click="setAllSelected()" wire:loading.attr="disabled" type="button"
                            class="ml-1 text-sm font-medium leading-5 text-blue-600 text-gray-700 underline transition duration-150 ease-in-out focus:text-gray-800 focus:underline focus:outline-none dark:text-white dark:hover:text-gray-400">
